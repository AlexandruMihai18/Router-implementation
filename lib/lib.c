...
{
	struct enq_packet *new_enq_packet = malloc(sizeof(struct enq_packet));
	new_enq_packet->next_hop_ip_addr = next_hop_ip_addr;
	
	new_enq_packet->buf = malloc(len);
	memcpy(new_enq_packet->buf, buf, len);

	new_enq_packet->len = len;

	return new_enq_packet;
}
